...
- 作業前に requirements スキル内の `references/requirements-guidelines.md` を読み、その品質基準(曖昧語禁止・検証可能性・MoSCoW原則・セルフレビュー手順)に従う。
- アプリに認証・決済・サーバーサイドDBのいずれかが含まれる場合は implement スキル内の `references/saas-guidelines.md` も読み、セクション0の適用判定を行って仕様書に明記し、セクション6の追加検討事項(利用規約・特商法表記・退会フロー・課金モデル等)を非機能要件に含める。
- スマホアプリの場合は implement スキル内の `references/mobile-guidelines.md` セクション0のプラットフォーム判定(PWA / ネイティブExpo)を行い、仕様書に「モバイル: PWA」または「モバイル: ネイティブ(Expo)— モバイルガイドライン適用」と明記する。ネイティブはストア配布・プッシュ通知等がMust要件のときのみ選ぶ。
