...
- スマホアプリの場合は `${CLAUDE_PROJECT_DIR}/.claude/skills/implement/references/mobile-guidelines.md` セクション0のプラットフォーム判定(PWA / ネイティブExpo)を行い、仕様書に「モバイル: PWA」または「モバイル: ネイティブ(Expo)— モバイルガイドライン適用」と明記する。ネイティブはストア配布・プッシュ通知等がMust要件のときのみ選ぶ。
